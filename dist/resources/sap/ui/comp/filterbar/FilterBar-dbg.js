...
sap.ui.define([
	...
	FilterBar.prototype._hasAnyVisibleText = function(oEndContent) {
		var oText = oEndContent.getContent()[0],
			bIsVisible = oText &&
						 oText.getVisible() &&
						 oText.getText() !== '';
		return !!bIsVisible;
	...
	FilterBar.prototype._hasAnyVisibleButtons = function() {
		var bHasVisibleButtons = this.getShowGoOnFB() ||
								 this.getShowClearOnFB() ||
								 this.getShowRestoreOnFB() ||
								 this.getShowFilterConfiguration();
		return !!bHasVisibleButtons;
	};

	FilterBar.prototype._toggleEndContentVisibility = function() {
		var oEndContent = this._oBasicAreaLayout && this._oBasicAreaLayout.getEndContent()[0],
			bHasVisibleContent;

		if (oEndContent) {
			bHasVisibleContent = this._hasAnyVisibleButtons() || this._hasAnyVisibleText(oEndContent);
			oEndContent.setVisible(bHasVisibleContent);
		}
	};
	...
	FilterBar.prototype.onBeforeRendering = function () {
		...
		if (!this.getIsRunningInValueHelpDialog() && this._isNewFilterBarDesign()) {
			this._toggleEndContentVisibility();
		}
	};
	...
	FilterBar.prototype._adaptButtonsEnablement = function() {
		...
		var bFlag = !!this._mAdvancedAreaFilter;
	...
	FilterBar.prototype.setUseToolbar = function(bValue) {
		...
		if (!bValue) {
			...
			this._oAdvancedPanel && this._oAdvancedPanel.setVisible(false);
		} else {
	...
	FilterBar.prototype._recreateToolbar = function() {
		...
		if (!this._oToolbar) {
			...
			if (!this._hasAnyVisibleFiltersOnFB()) {
				this._oAdvancedPanel.addContent(this._oHintText);
	...
	FilterBar.prototype._addButtonsToBasicArea = function() {
		...
		//TODO: Can we remove the Text control?
...
//			}
	...
	FilterBar.prototype._toggleHideShow = function() {
		...
		if (this.isPropertyInitial("showAllFilters")) {
			bShowAllFilters = this._hasMandatoryFields() ? true : false;
			this.setShowAllFilters(bShowAllFilters);
		}
						...
						for (var i = 0; i < oGroup.items.length; i++) {
							...
							if (oItem.filterItem && !oItem.filterItem.getHiddenFilter() && oItem.control && oItem.container) {
								...
								if (oItem.contentReplaced) {
									...
									oItem.container.getContent()[0].setLabelFor && oItem.container.getContent()[0].setLabelFor(oItem.control);
				...
				if (!this._oAdaptFiltersDialog) {
					...
					oFilterPanel.setItemFactory(function(oContext) {
						...
						if (!oItem.contentReplaced && oItem.container && oItem.container.getContent()[1] === oControl) {
							...
							oItem.container.getContent()[0].setLabelFor && oItem.container.getContent()[0].setLabelFor("");
...
//						}
					...
					sap.ui.require(["sap/m/p13n/Popup"], function(P13nPopup){
						var oAdaptFilterDialog = new P13nPopup(this.getId() + "-adapt-filters", {
							...
							warningText: sap.ui.getCore().getLibraryResourceBundle("sap.m").getText("p13n.RESET_WARNING_TEXT"),
							close: function(oEvt) {
	...
	FilterBar.prototype._createButtons = function(oToolbar) {
		...
		this._oSearchButton.addEventDelegate({
	...
	FilterBar.prototype._toggleAllFilters = function() {
		...
		this._oShowAllFiltersButton.setVisible(!this.getShowAllFilters());
		this._rerenderFilters();
...
// }
	...
	FilterBar.prototype._rerenderFilters = function() {
		...
		var bShouldDisplayFirstNFilters = this.getIsRunningInValueHelpDialog() && !this.getShowAllFilters();
			...
			if (aVisibleFiltersFieldArray.length > 0) {
				for (var i = 0; i < aVisibleFiltersFieldArray.length; i++) {
					var oCurrentField = aVisibleFiltersFieldArray[i],
						oControl = oCurrentField.getControl(),
